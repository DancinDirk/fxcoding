...
//+------------------------------------------------------------------+
//| Custom inputs for controlling the labels                         |
//+------------------------------------------------------------------+
enum CornerEnum
{
   CORNER_TOP_LEFT,
                ...
                const int &spread[])
{
   ...
   if (Net_Trades > 0 && Net_Lots != 0)
   {
      ...
      if (Net_Lots > 0)                                                                                                           // Long position
      {
         ...
         if (Average_Price > tpPrice)
         {
            pipsOfProfit = (tpPrice - Average_Price) / point;
         } // if loss @ TP, distance, in pips, from breakeven price to TP price is negative
         else
         {
            pipsOfProfit = MathAbs(tpPrice - Average_Price) / point;
         }
      ...
      if (Net_Lots < 0) // Short position
      {
         ...
         if (Average_Price < tpPrice)
         {
            pipsOfProfit = (Average_Price - tpPrice) / point;
         } // if loss @ TP, distance, in pips, from breakeven price to TP price is negative
         else
         {
            pipsOfProfit = MathAbs(Average_Price - tpPrice) / point;
         }
   ...
   if (Average_Price > 0)
   {
      ...
      {
         ppMultiplier = Net_Result / pipsToBE;
      }; // price per pip multiplier
